...
class BookNavigation(QWidget):
    ...
    def __init__(self, book_master:BookMaster=None):
        ...
        exit_btn.clicked.connect(lambda: self.exit_navigation_signal.emit(self))
    ...
    def queryBookmaster(self):
        split_order, split_not_present_in_order_list = self.bookmaster.getListOfAllSplits()
    ...
    def loadSpecificSplit(self, split_name:str) -> None:    # loades the split order
        self.load_specific_split_signal.emit(split_name)
        self.exit_navigation_signal.emit(self)
